...
**A cron trigger needs a `scheduled` handler.** The worker generated by
`@opennextjs/cloudflare` exports only `fetch`. Declaring `triggers.crons` in
`wrangler.jsonc` against that worker does nothing at all — the trigger fires, finds no
`scheduled` export, and the scan never runs, while the deploy and the dashboard both look
healthy. `worker.ts` is therefore a **custom entrypoint**: it re-exports OpenNext's fetch
handler and adds the missing `scheduled` handler, which drives the same code path as
`GET /api/cron/scan`. `wrangler.jsonc` points `main` at `worker.ts`, not at
`.open-next/worker.js`.
